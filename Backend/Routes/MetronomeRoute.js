...
Metronome.post("/", async (request, response) => {
    try {
        ...
        // update server
        return response.status(201).json({created : newSongJoined});
    } catch (error) {
...
Metronome.get("/", async (request, response) => {
    try {
        const SongsFound = await SongMetronome.find({});

        return response.status(200).json({list: SongsFound}); // problems w/ wrapper'?
    } catch (error) {
        console.log(error.Message);
